...
for N in ${array1[@]}; do
	...
	valgrind --tool=cachegrind -q --cachegrind-out-file=normalTmp.dat ./ej3 normal $N #>/dev/null
	echo "Ejecutando valgrind para medir los fallos de cache de las traspuestas"
	valgrind --tool=cachegrind -q --cachegrind-out-file=traspTmp.dat ./ej3 traspuesta $N #>/dev/null
...
gnuplot << END_GNUPLOT
set title "Matrix Multiplication Data Write Cache Misses"
set ylabel "Misses"
...
plot "$fDAT" using 1:4 with lines lw 2 title "./normal write misses", \
...
gnuplot << END_GNUPLOT
set title "Matrix Multiplication Data Read Cache Misses"
set ylabel "Misses"
set xlabel "Matrix Size"
set key right bottom
set grid
